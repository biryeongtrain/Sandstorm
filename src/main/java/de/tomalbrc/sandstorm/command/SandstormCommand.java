...
public class SandstormCommand {
    public static final Permission PERMISSION = Permission.Atom.create(Identifier.fromNamespaceAndPath("sandstorm", "command"));
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        var sandstormNode = Commands.literal("sandstorm").requires(ctx -> ctx.permissions().hasPermission(PERMISSION) || ctx.permissions().hasPermission(new Permission.HasCommandLevel(PermissionLevel.MODERATORS))).executes(x -> {
            x.getSource().sendSystemMessage(Component.literal("Sandstorm 0.1-alpha by tomalbrc a.k.a. Pinnit"));
            return Command.SINGLE_SUCCESS;
        });

        sandstormNode.then(argument("effect", IdentifierArgument.id()).suggests(new ParticleSuggestionProvider()).executes(SandstormCommand::execute));
        sandstormNode.then(argument("effect", IdentifierArgument.id()).suggests(new ParticleSuggestionProvider()).then(argument("position", Vec3Argument.vec3()).executes(SandstormCommand::executeAt)));
        sandstormNode.then(literal("clear").executes(SandstormCommand::clear));

        dispatcher.getRoot().addChild(sandstormNode.build());
    }
    ...
    private static int execute(CommandContext<CommandSourceStack> context) {
        Identifier effectString = IdentifierArgument.getId(context, "effect");
